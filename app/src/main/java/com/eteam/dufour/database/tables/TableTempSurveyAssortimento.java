...
// This table is used to store old values for dashboard. In dashboard the values in this table and TableTempSurveyAssortimento are compared.
//(This table contains the old values before any changes made)
public class TableTempSurveyAssortimento extends DBTable {
    // ===========================================================
    // Constants
    // ===========================================================

    private static final String TAG = TableTempSurveyAssortimento.class.getName();

    public static final String KEY_ARRAY_NAME = "ASSORT";
    public static final String KEY_JSON_ARRAYNAME = "assortimento";

    public static final String TABLE_NAME = "temp_survey_assortimento";
    ...
    public static void joinDuplicates(SQLiteDatabase db, long surveyId) {
        Cursor c = db.rawQuery("SELECT " + TableTempSurveyAssortimento.COLUMN_ITEM_CODE + " FROM "
                + TableTempSurveyAssortimento.TABLE_NAME
                + " WHERE " + TableTempSurveyAssortimento.COLUMN_SURVEY_ID + "=" + surveyId
                + " GROUP BY " + TableTempSurveyAssortimento.COLUMN_ITEM_CODE
                + " HAVING COUNT(" + TableTempSurveyAssortimento.COLUMN_ITEM_CODE + ")>1", null);
        if (c.moveToFirst()) {
            while (!c.isAfterLast()) {
                String itemCode = c.getString(c.getColumnIndex(TableTempSurveyAssortimento.COLUMN_ITEM_CODE));
    ...
    private static void convergeAllItems(SQLiteDatabase db, String itemCode, long surveyId) {
        Cursor c = db.query(TableTempSurveyAssortimento.TABLE_NAME, getConvergeColumns()
                , TableTempSurveyAssortimento.COLUMN_ITEM_CODE + "=" + itemCode
                        + " AND " + TableTempSurveyAssortimento.COLUMN_SURVEY_ID + "=" + surveyId, null
                , null, null, TableTempSurveyAssortimento.COLUMN_ID + " DESC");
        ...
        if (c.moveToFirst()) {
            boolean first = true;
            while (!c.isAfterLast()) {
                int id = c.getInt(c.getColumnIndex(TableTempSurveyAssortimento.COLUMN_ID));
...
//				idsToDelete = String.valueOf(id);
        ...
        if (!idsToDelete.isEmpty() && idToUpdate != -1) {
            ...
            db.delete(TableTempSurveyAssortimento.TABLE_NAME, TableTempSurveyAssortimento.COLUMN_ID + " IN (?)", new String[]{idsToDelete});
            ContentValues values = new ContentValues();
            values.put(TableTempSurveyAssortimento.COLUMN_SURVEY_PREZZO, product.prezzo);
            values.put(TableTempSurveyAssortimento.COLUMN_SURVEY_PREZZO_PROMO, product.prezzoPromo);
            values.put(TableTempSurveyAssortimento.COLUMN_SURVEY_FACING, product.facing);
            //   values.put(TableTempSurveyAssortimento.COLUMN_SURVEY_FUORI_BANCO, product.fb);
            values.put(TableTempSurveyAssortimento.COLUMN_SURVEY_LEVEL, product.assorti);
            values.put(TableTempSurveyAssortimento.COLUMN_SURVEY_LINEA, product.prezzoLinea);
            db.update(TableTempSurveyAssortimento.TABLE_NAME, values, TableTempSurveyAssortimento.COLUMN_ID + "=" + idToUpdate, null);
        }
...
//			Log.d(TAG, "Survey is updated and id is"+survey.id+" for item "+survey.itemCode);
    ...
    public static void delete(ElahDBHelper dbHelper, long surveyId, String itemCode) {
        dbHelper.getWritableDatabase().delete(TableTempSurveyAssortimento.TABLE_NAME,
                TableTempSurveyAssortimento.COLUMN_SURVEY_ID + "=" + surveyId
                        + " AND " + TableTempSurveyAssortimento.COLUMN_ITEM_CODE + "='" + itemCode + "'", null);
    }

    public static void delete(long surveyId) {
        getDB().delete(TableTempSurveyAssortimento.TABLE_NAME,
                TableTempSurveyAssortimento.COLUMN_SURVEY_ID + "=" + surveyId, null);
    }

    public static void delete(String idToDelete) {
        getDB().delete(TableTempSurveyAssortimento.TABLE_NAME,
                TableTempSurveyAssortimento.COLUMN_SURVEY_ID + " IN (" + idToDelete + ")", null);
    }

    public static ArrayList<String> getItemCodes(long surveyId) {
        ArrayList<String> array = new ArrayList<String>();
        Cursor c = getDB().query(TableTempSurveyAssortimento.TABLE_NAME, new String[]{TableTempSurveyAssortimento.COLUMN_ITEM_CODE},
                TableTempSurveyAssortimento.COLUMN_SURVEY_ID + "=" + surveyId, null, null, null, null);

        if (c.moveToFirst()) {
            while (!c.isAfterLast()) {
                array.add(c.getString(c.getColumnIndex(TableTempSurveyAssortimento.COLUMN_ITEM_CODE)));
                c.moveToNext();
            }
        }
        c.close();
...
//		Log.d("Log", "Population started for "+surveyId+"in Prodotti Survey operation");
        Cursor c = getDB().query(TableTempSurveyAssortimento.TABLE_NAME,
                new String[]{TableTempSurveyAssortimento.COLUMN_ID
                        , TableTempSurveyAssortimento.COLUMN_SURVEY_ID
                        , TableTempSurveyAssortimento.COLUMN_ITEM_CODE
                        , TableTempSurveyAssortimento.COLUMN_ITEM_DESC
                        , TableTempSurveyAssortimento.COLUMN_ITEM_MARCHIO
                        , TableTempSurveyAssortimento.COLUMN_ITEM_LINEA
                        , TableTempSurveyAssortimento.COLUMN_ITEM_MACROFAMIGLIA
                        , TableTempSurveyAssortimento.COLUMN_SURVEY_PREZZO
                        , TableTempSurveyAssortimento.COLUMN_SURVEY_PREZZO_PROMO
                        , TableTempSurveyAssortimento.COLUMN_SURVEY_FACING
                        , TableTempSurveyAssortimento.COLUMN_SURVEY_LINEA
                        //   , TableTempSurveyAssortimento.COLUMN_SURVEY_FUORI_BANCO
                        , TableTempSurveyAssortimento.COLUMN_SURVEY_LEVEL
                        , TableTempSurveyAssortimento.COLUMN_UNIQUE_FIELD
                        , TableTempSurveyAssortimento.COLUMN_SURVEY_ASSORT_MODIFY},
                TableTempSurveyAssortimento.COLUMN_SURVEY_ID + "=" + surveyId, null, null, null, null);
    ...
    public static boolean isAllMandatoryFieldsFilled(long surveyId) {


        //fields are not mandatory if survey level is 2 (NonAssortito)

        String selection = TableTempSurveyAssortimento.COLUMN_SURVEY_ID + "=" + surveyId + " AND ("
        ...
        Cursor c = getDB().query(TableTempSurveyAssortimento.TABLE_NAME,
                new String[]{TableTempSurveyAssortimento.COLUMN_SURVEY_ID
                        , TableTempSurveyAssortimento.COLUMN_ID //
                        , TableTempSurveyAssortimento.COLUMN_ITEM_CODE //
                        , TableTempSurveyAssortimento.COLUMN_SURVEY_PREZZO
                        , TableTempSurveyAssortimento.COLUMN_SURVEY_PREZZO_PROMO
                        , TableTempSurveyAssortimento.COLUMN_SURVEY_FACING
                        , TableTempSurveyAssortimento.COLUMN_SURVEY_LINEA
                        , TableTempSurveyAssortimento.COLUMN_SURVEY_LEVEL
                        , TableTempSurveyAssortimento.COLUMN_SURVEY_ASSORT_MODIFY}, selection
...
//		Log.d("Log", "Population started for "+surveyId+"in Prodotti Survey operation");
        Cursor c = getDB().query(TableTempSurveyAssortimento.TABLE_NAME,
                new String[]{
                        ...
                        , TableTempSurveyAssortimento.COLUMN_ITEM_LINEA
                        , TableTempSurveyAssortimento.COLUMN_SURVEY_LEVEL},
                TableTempSurveyAssortimento.COLUMN_ITEM_LINEA + "=" + "\""+linea+"\""
                        + " AND " + TableTempSurveyAssortimento.COLUMN_SURVEY_ID + "=" +  surveyId+ " AND " + TableTempSurveyAssortimento.COLUMN_SURVEY_LEVEL + "=" +  surveyLevel, null, null, null, null);
...
//		Log.d("Log", "Population started for "+surveyId+"in Prodotti Survey operation");
        Cursor c = getDB().query(TableTempSurveyAssortimento.TABLE_NAME,
                new String[]{
                        ...
                        , TableTempSurveyAssortimento.COLUMN_SURVEY_LEVEL},
                TableTempSurveyAssortimento.COLUMN_ITEM_LINEA + "=" + "\""+linea+"\""
                        + " AND " + TableTempSurveyAssortimento.COLUMN_SURVEY_ID + "=" +  surveyId, null, null, null, null);
    ...
    public static class SurveyAssortimento implements Parcelable {
        ...
        public static final Creator<SurveyAssortimento> CREATOR
                = new Creator<SurveyAssortimento>() {
...
//			Log.d(TAG, "Changing facing value "+facing);
     ...
     /*   public void setFb(int fb) {
        ...
        private void updateBaseValues() {
            updateModifiedTime();
            values.put(TableTempSurveyAssortimento.COLUMN_SURVEY_ID, surveyId);
            values.put(TableTempSurveyAssortimento.COLUMN_ITEM_CODE, itemCode);
            values.put(TableTempSurveyAssortimento.COLUMN_ITEM_DESC, itemDesc);
            values.put(TableTempSurveyAssortimento.COLUMN_ITEM_LINEA, itemLinea);
            values.put(TableTempSurveyAssortimento.COLUMN_ITEM_MARCHIO, itemMarchio);
            values.put(TableTempSurveyAssortimento.COLUMN_ITEM_MACROFAMIGLIA, itemMacrofamiglia);
            values.put(TableTempSurveyAssortimento.COLUMN_UNIQUE_FIELD, uniqueId);
            values.put(TableTempSurveyAssortimento.COLUMN_SURVEY_ASSORT_MODIFY, surveyModified);
        }
        ...
        public ContentValues getValues() {
            ...
            values.put(TableTempSurveyAssortimento.COLUMN_SURVEY_PREZZO, Util.getAsElahDBNumFormat(prezzo));
            values.put(TableTempSurveyAssortimento.COLUMN_SURVEY_PREZZO_PROMO, Util.getAsElahDBNumFormat(prezzoPromo));
            values.put(TableTempSurveyAssortimento.COLUMN_SURVEY_LINEA, prezzoLinea);
            values.put(TableTempSurveyAssortimento.COLUMN_SURVEY_ID, surveyId);
            //   values.put(TableTempSurveyAssortimento.COLUMN_SURVEY_FUORI_BANCO, fb);
            values.put(TableTempSurveyAssortimento.COLUMN_SURVEY_LEVEL, assorti);
            values.put(TableTempSurveyAssortimento.COLUMN_SURVEY_FACING, facing);
            values.put(TableTempSurveyAssortimento.COLUMN_UNIQUE_FIELD, uniqueId);
            values.put(TableTempSurveyAssortimento.COLUMN_SURVEY_ASSORT_MODIFY, surveyModified);
        ...
        public JSONObject getAsJson() throws JSONException {
            ...
            survey.put(TableTempSurveyAssortimento.COLUMN_ITEM_CODE, formatStringToJSON(itemCode));
            survey.put(TableTempSurveyAssortimento.COLUMN_SURVEY_ID, surveyId);
            survey.put(TableTempSurveyAssortimento.COLUMN_ITEM_DESC, formatStringToJSON(itemDesc));
            survey.put(TableTempSurveyAssortimento.COLUMN_ITEM_MARCHIO, formatStringToJSON(itemMarchio));
            survey.put(TableTempSurveyAssortimento.COLUMN_ITEM_LINEA, formatStringToJSON(itemLinea));
            survey.put(TableTempSurveyAssortimento.COLUMN_ITEM_MACROFAMIGLIA, formatStringToJSON(itemMacrofamiglia));
            survey.put(TableTempSurveyAssortimento.COLUMN_SURVEY_PREZZO, formatStringToJSON(prezzo));
            survey.put(TableTempSurveyAssortimento.COLUMN_SURVEY_PREZZO_PROMO, formatStringToJSON(prezzoPromo));
            survey.put(TableTempSurveyAssortimento.COLUMN_SURVEY_FACING, facing);
            survey.put(TableTempSurveyAssortimento.COLUMN_SURVEY_LINEA, formatStringToJSON(prezzoLinea));
            //   survey.put(TableTempSurveyAssortimento.COLUMN_SURVEY_FUORI_BANCO, fb);
            survey.put(TableTempSurveyAssortimento.COLUMN_SURVEY_LEVEL, assorti);
            survey.put(TableTempSurveyAssortimento.COLUMN_UNIQUE_FIELD, formatStringToJSON(uniqueId));
            survey.put(TableTempSurveyAssortimento.COLUMN_SURVEY_ASSORT_MODIFY, formatStringToJSON(surveyModified));
